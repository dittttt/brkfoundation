...
old_str = r"            \{\/\* RIGHT: Live Preview Column \*\/\}"
new_str = """              {/* Revision History */}
              {editingPost.revisions && editingPost.revisions.length > 0 && (
                <div className="mt-auto px-6 md:px-8 border-t border-gray-100 pt-6 pb-6 bg-slate-50 border-b border-gray-100">
                    ...
                    {editingPost.revisions.map((rev: any, idx: number) => (
                      <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-white border border-gray-200 shadow-sm rounded-xl">
                        <div>
                          ...
                          <p className="text-[10px] text-gray-500">Edited by {rev.editor || 'Admin'}</p>
                        </div>
                        <button onClick={() => {
...
# fix the extra closing div issue
if """              )}
            </div>

            {/* RIGHT: Live Preview Column */}""" in res:
